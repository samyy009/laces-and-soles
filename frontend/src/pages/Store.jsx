...
export default function Store() {
  ...
  return (
    <div className="pb-16 bg-white min-h-screen">
      ...
      <section className="relative h-[180px] flex items-center bg-gray-50 overflow-hidden border-b border-gray-100">
         <div 
         ...
         <div className="relative z-10 w-full text-center px-6">
            <h1 className="text-3xl md:text-4xl font-black uppercase text-gray-900 tracking-tighter leading-none mb-3">
      ...
      <section className="py-4 mx-auto max-w-[1400px] px-6">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-12 py-8 flex flex-col md:flex-row gap-8 lg:gap-12">
        {/* Mobile Header for Filters/Summary */}
        <div className="md:hidden flex items-center justify-between bg-white  p-4 rounded-2xl border border-gray-100 shadow-sm mb-2">
        ...
        <aside className="hidden md:block md:w-64 flex-shrink-0 space-y-8 sticky top-28 self-start">
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3">SEARCH CURATED</h4>
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 border border-gray-100 rounded-2xl py-3 pl-12 pr-4 text-sm font-semibold outline-none focus:border-[#ff3366] transition-colors"
                />
              </div>

            <div>
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-3">SELECT BRAND</h4>
              <ul className="space-y-2">
                {brands.map(brand => (
                  <li key={brand}>
                    <button 
                      onClick={() => setActiveFilter(brand)}
                      className={`flex items-center justify-between w-full text-sm font-black tracking-widest uppercase transition-colors ${activeFilter === brand ? 'text-[#ff3366]' : 'text-gray-900 hover:text-[#ff3366]'}`}
                    >
                      {brand}
                      {activeFilter === brand ? (
          ...
          <div className="flex-1">
             <div className="flex flex-col sm:flex-row items-center justify-between mb-5 pb-4 border-b border-gray-100 gap-4">
                ...
                <div className="flex items-center gap-4 sm:gap-6 justify-center w-full sm:w-auto">
                  <div className="flex bg-gray-50 p-1 rounded-xl">
                      <button onClick={()=>setViewMode('grid')} className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-white shadow-sm text-[#ff3366]' : 'text-gray-400'}`}><Icons.LayoutGrid size={18} /></button>
                      <button onClick={()=>setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-white shadow-sm text-[#ff3366]' : 'text-gray-400'}`}><Icons.List size={18} /></button>
                  </div>
             ...
             <div className={viewMode === 'grid' ? "grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-8" : "space-y-4 sm:space-y-8"}>
              {filteredProducts.map((product) => (
                <div key={product.id} className="group relative border border-gray-100 rounded-[32px] overflow-hidden hover:shadow-xl transition-shadow duration-300 p-2 bg-white flex flex-col">
                    <div className="relative group/card overflow-hidden rounded-[24px]">
                       <button 
